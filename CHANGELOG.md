...
### 改善
- **貢献速報（月次）** 日付行・月ナビバー・事業所列をスクロール時に固定
  - `thead th` に `position:sticky` を追加（日付ヘッダーが縦スクロールで消えない）
  - 月ナビバー（ツールバー）を topbar 直下に sticky 固定
  - 事業所列は既存の左 sticky に加え、topbar＋toolbar 分のオフセットを設定
  - `.cal-wrap` の `overflow:hidden` を `overflow:clip` に変更（sticky が正常に機能するよう修正）

---

## v1.19.2 (2026-09-10〜16)

### 修正
- **salary.html（給与CSV取込）** 「処遇緊急支援/補助」を自動的に支給合計から除外
  - 処遇緊急支援加算・補助関連の行は報酬単価として扱わず、支給合計の集計から自動除外
- **invoice.html** PDF明細書の自動読取で回数を誤認識するバグを修正

### 改善
- **invoice.html** 請求月・事業所セレクタの初期表示を高速化

---

## v1.19.1 (2026-09-08〜09)

### 変更
- **賃金台帳の閲覧権限（salary.view）を廃止し全員に開放**
  - 従来：`salary.view` 権限を持つユーザーのみ閲覧可
  - 変更後：ログインユーザー全員が閲覧可能

### 改善
- **report.html（貢献実績表）** 所属未設定ユーザーのデフォルト画面を全社表示に変更
- **report.html（貢献実績表）** 全社表示の事業所折りたたみを事業所単独モードと同じフル版に変更

---

## v1.19.0 (2026-09-06〜07)

### 新機能
- **report.html（貢献実績表）** 「分析」モードを追加
  - 損益分岐点・稼働率分析：年度累計の事業所別達成率一覧（経過月のみ集計）
  - 営業ファネル：問合せ・見学・契約の推移
  - サイドバーに「分析」メニューを追加
- **leads.html（問合せ・見学リード管理）** 新規作成
  - 問合せ・見学の日付・事業所・状況・コメントを記録・一覧管理
- **index.html（ポータル）** PC画面に共通サイドバー＋トップバーを適用

### 修正
- **daily.html** 日次報告メインフォームで未編集の `kids:0` を書き込まないよう修正
- **morning-report.html** 週次予定入力で新規保存時に `kids:0` を書き込まないよう修正
...
### 新機能：サイドバー共通化
- **sidebar.js / sidebar.css（新規）** サイドバー・ユーザーバッジを共通コンポーネントとして切り出し
  - メニューをセクション分け（速報入力 / レポート / プロジェクト / 管理）
  - PC：左固定サイドバー表示、スマホ：ハンバーガー開閉
- **全ページ（report / daily / monthly / morning-report / mirai-todo / import / salary / invoice / settings）** にサイドバーを展開

### 新機能：貢献実績表 月別推移グラフ
- **report.html** 月別推移グラフ（キャンセル分析）を追加
  - 左グラフ：実績人数の積み上げ棒（達成度で色分け）＋目標・損益分岐線
  - 右グラフ：予定×実績の積み上げ棒＋キャンセル人数・概算損失月別表
  - サマリーカード：目標・損益・キャンセルの3カテゴリ
  - グラフ・表はトグル開閉、サマリーは常時表示

### 新機能：実質目標人数
- **settings.html** 事業所ごとに「実質目標人数」（定員と別）を設定できるように
- **monthly.html** 月次カレンダーの予定/目標に実質目標人数を反映（未設定時は定員を使用）

### 新機能：マスコット「ベルる」演出
- 読み込み中・データなし表示にベルるアイコンのバウンスアニメーションを追加（全ページ共通）
- **daily.html** 日次報告後にベルるの表情演出（目標達成で笑顔、遅れで困り顔）とお祝いポップアップを追加
  - ポップアップは全員へのねぎらいメッセージに変更（目標達成者限定→全員）

### 改善
- **settings.html** テーブル1列目を固定（sticky）、PC幅が十分な時はコンテンツ幅を拡大
- **report.html** 所属事業所を持つユーザーには事業所×現在年度をデフォルト表示

---

## v1.17.0 (2026-09-04)

### 新機能：貢献実績表 PDF/Excel出力
- **report.html** PDF出力機能を追加（全情報PDF保存）
  - 印刷CSS対応、事業所・エリア・全社ごとに改ページ
- **report.html** Excel出力機能を追加（全情報Excel保存）
  - 事業所別・エリア別・全社をシート分け、装飾・結合つき
- **report.html** エリア集計・Excel出力で複数サービス提供事業所を種別ごとに分割表示

### 新機能：給与取込 CSV対応
- **salary.html** 給与取込をPDFからCSV専用に切り替え（従来のPDF解析方式を廃止）
- **index.html** ポータルに「給与取込」タイルを追加（admin/supporter限定）

### 改善
- **report.html（事業所モード）** サービス別内訳をエリア集計と同じ下部別表形式に統一、加算ごと→事業ごとの並びに変更、貢献人数／日を追加

### 修正
- **report.html** サービス別内訳表が基本報酬しか表示されていなかった不具合を修正
- **report.html** サービス別内訳表の処遇改善加算行の列ずれ（rowspan不足）を修正
- **report.html** ラフォーレ亀山の部課コード変更（302→311）により7月給与が反映されない不具合を修正

---

## v1.16.0 (2026-09-01〜03)

### 新機能
- **invoice.html** CSVにない利用者を個別明細書（PDF自動読取 / 手動入力）から追加できる機能を追加

### 改善
- **invoice.html** 地域単価を事業所名ベースの確定テーブルに変更（利用者ごとの地域区分で個別計算する方式に統一）

### 修正
- **invoice.html** PDF自動読み取りで明細行が抜け落ちる不具合を修正
- **invoice.html** 個別明細書追加フォームで事業所が選択できない競合状態を修正
- **invoice.html** はれのいえ・にじのいえの地域単価を事業所固定でその他10円に修正
...
### 修正
- **invoice.html** 地域単価を利用者ごとの地域区分で個別計算するよう修正
...
### 新機能
- **mirai-todo.html（未来創造企業 ToDo）** 部署・担当者フィルタを追加

### 改善
- **invoice.html** 保育所等訪問支援の単価・表示区分を追加
- **invoice.html** 未登録コードの表示に事業所・提供月の内訳を追加
...
### 新機能
- **monthly.html（貢献速報 月次）** 月別カレンダーにキャンセル人数（予定－実績）の月計列を追加
- **monthly.html** 月計サマリの列順序を整理し、キャンセル概算金額を追加
